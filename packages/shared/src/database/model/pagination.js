...
class SequelizePaginate {
  ...
  paginate(Model) {
    ...
    const pagination = async function ({
      ...
      let containsWhereInInclude = false;

      if (options['include']) {
        for (const element of options.include) {
          if (checkForWhereClause(element)) {
            containsWhereInInclude = true;
            break;
          }
        }
      }
      if (!containsWhereInInclude) {
        countOptions = Object.keys(options).reduce((acc, key) => {
    ...
    function checkForWhereClause(item) {
      if (item.where) {
        return true;
      }
      if (item.include) {
